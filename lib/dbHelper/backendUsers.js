...
module.exports = {
     ...
     * @param next          回调
     */
    updatePassword : function (id, loginPassword, next) {
        var sql = 'UPDATE BackendUsers SET LoginPassword=? WHERE ID=?';
        var parameters = [loginPassword, id];
        dbConn.query(sql, parameters, function(err, result) {
            if (err) throw err;
            next && next(result);
        });
     ...
     * @param ip
     */
    updateLoginInfo : function (id, ip, next) {
