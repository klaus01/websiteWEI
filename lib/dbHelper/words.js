...
function likeFind(fieldName, value, offset, resultCount, next) {
    ...
    WHERE w.CreateUserID=u.AppUserID AND w.' + fieldName + ' LIKE ? ORDER BY UseCount_Before1D_CN';
    var parameters = ['%' + value + '%'];
    if (offset) {
        sql += ' LIMIT ?, ?';
...
module.exports.find = function (offset, resultCount, next) {
    if (!next && typeof(offset) === 'function') {
        next = offset;
        offset = null;
    }
    var sql = 'SELECT * FROM Words ORDER BY UseCount_Before1D_CN';
    var parameters = null;
    if (offset) {
        sql += ' LIMIT ?, ?';
        parameters = [offset, resultCount];
    }
    dbConn.query(sql, parameters, function (err, rows) {
        if (err) throw err;
        rows = publicFunction.addWordPictureAndAudioUrl(rows);
