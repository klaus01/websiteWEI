...
function resultJSON(res, isSuccess, content, callback) {
    var ret = {
        success: isSuccess
    };
    if (isSuccess)
    ...
    if (callback) {
        res.end(callback + '&&' + callback + '(' + JSON.stringify(ret) + ')');
    }
    else {
        res.json(ret);
    }
}
module.exports.error = function (res, message, callback) {
    resultJSON(res, false, message, callback);
};
module.exports.success = function (res, data, callback) {
    resultJSON(res, true, data, callback);
};
 ...
 * @returns {Boolean}
 */
module.exports.isNum = function (v) {
    if (typeof(v) === 'number')
        return !isNaN(v);
    return false;
};
...
module.exports.moveAppUserIconFile = function (appUserID, file, next){
    var sourcePath = path.join(__dirname, '..', file.path);
    var targetPath = path.join(__dirname, '..', 'public', settings.appUserIconsDir, (appUserID % 10).toString(), file.name);
...
module.exports.moveActivityPictureFile = function (partnerUserID, file, next){
    var sourcePath = path.join(__dirname, '..', file.path);
    var targetPath = path.join(__dirname, '..', 'public', settings.activityPicturesDir, partnerUserID, file.name);
    movefile(sourcePath, targetPath,
        function(err) {
            if (err) {// 删除临时文件夹文件
                fs.unlink(sourcePath, function (err) {
...
module.exports.moveWordPictureFile = function (wordID, file, next){
    var sourcePath = path.join(__dirname, '..', file.path);
    var targetPath = path.join(__dirname, '..', 'public', settings.wordPicturesDir, (wordID % 10).toString(), file.name);
    movefile(sourcePath, targetPath,
        function(err) {
            if (err) {// 删除临时文件夹文件
                fs.unlink(sourcePath, function (err) {
...
module.exports.moveWordAudioFile = function (wordID, file, next){
    var sourcePath = path.join(__dirname, '..', file.path);
    var targetPath = path.join(__dirname, '..', 'public', settings.wordAudioDir, (wordID % 10).toString(), file.name);
    movefile(sourcePath, targetPath,
        function(err) {
            if (err) {// 删除临时文件夹文件
                fs.unlink(sourcePath, function (err) {
