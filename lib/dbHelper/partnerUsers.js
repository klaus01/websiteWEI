...
module.exports.findMessagesBySubscriberID = function (value, next) {
 ...
 ORDER BY m.LastTime',
        ...
        for (var i = 0; i < rows.length; i++) {
            var row = rows[i];
            var partnerUser = publicFunction.addPartnerUserIconUrl(row.pu);
            partnerUser.LastMessageTime = row.m.LastTime;
            partnerUser.UnreadCount = row.m.UnreadCount;
            partnerUser.NoAwardCount = row.m.NoAwardCount;
            rows[i] = partnerUser;
        }
        next && next(rows);
    });
